...
if user_menu == 'Country-wise Analysis':
    ...
    st.title(selected_country + " across various sports: ")
    ...
    top_15_df = helper.most_successful_countrywise(df,selected_country)
    st.table(top_15_df)
...
if user_menu == 'Athlete-wise Analysis':
    ...
    st.title('Height vs Weight of Athletes(ScatterPlot)')
    selected_sport = st.selectbox(("Select a Sport "), sport_list)
    ...
    st.title('Height vs Weight of Athletes(LinePlot)')
    selected_sport = st.selectbox(("Select a Sport "), sport_list, key = "count")
    temp_df = helper.line_weight_v_height(df, selected_sport)
    fig, ax = plt.subplots()
    ax = sns.lineplot(x = temp_df['Weight'], y = temp_df['Height'], hue=temp_df['Medal'])
    st.pyplot(fig)

    st.title('Age vs Medals of Athletes for each Sport')
    selected_sport = st.selectbox(("Select a Sport "), sport_list, key = 'ctr')
    temp_df = helper.age_vs_weight(df, selected_sport)
    fig, ax = plt.subplots()
    ax = sns.barplot(x = temp_df['Medal'], y = temp_df['Age'], hue=temp_df['Sex'])
    st.pyplot(fig)
